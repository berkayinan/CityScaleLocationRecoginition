...
for subsetID=1:10
    ...
    relativeIndex=1;
    for locationID=((subsetSize*(subsetID-1))+1):(subsetSize*(subsetID))
        fprintf('Occurance: Location ID: %d\n',locationID);
        for position=1:nPositions
            ...
            occurence(:,ceil(relativeIndex/nPositions))=occurence(:,ceil(relativeIndex/nPositions))+realHistogram;
        end
    ...
    for locationID=((subsetSize*(subsetID-1))+1):(subsetSize*(subsetID))
        ...
        for position=1:nPositions
           ...
           for k=1:length(linIndex)
             ...
             entropy(k)=calculateEntropy(numberOfImagesInSubTree,nPositions,occurence(linIndex(k),ceil(relativeIndex/5)),totalOcc-occurence(linIndex(k),ceil(relativeIndex/5)));
           end
